...
public class MonthAttendanceGen extends ServerResource {
	public MonthAttendanceGen() {
	...
	protected Representation post(Representation entity, Variant variant)
		...
		try {
			...
			final int percentageRow = noOfRows;
			...
			Utils.helpBatch(inEntries.get(noOfCol * (presentRow - 1)), outFeed,
					"No present");
			Utils.helpBatch(inEntries.get(noOfCol * (absentRow - 1)), outFeed,
					"No absent");
			Utils.helpBatch(inEntries.get(noOfCol * (percentageRow - 1)),
					outFeed, "Present %");
			...
			for (i = 0; i < maxDate; i++) {
				...
				Utils.helpBatch(
						inEntries.get(noOfCol * (absentRow - 1) + i + 2),
						outFeed,
						absentRowString.replaceAll("\\@",
								Utils.getColumn(i + 3, 0)));
				Utils.helpBatch(
						inEntries.get(noOfCol * (presentRow - 1) + i + 2),
						outFeed,
						presentRowString.replaceAll("\\@",
								Utils.getColumn(i + 3, 0)));
				Utils.helpBatch(
						inEntries.get(noOfCol * (percentageRow - 1) + i + 2),
						outFeed,
			...
			for (i = 0; i < strength; i++) {
				CustomElementCollection elements = studEntries.get(i)
						.getCustomElements();
				Utils.helpBatch(inEntries.get(noOfCol * (i + 3)), outFeed,
						"=\"" + elements.getValue("studentid") + "\"");
				Utils.helpBatch(inEntries.get(noOfCol * (i + 3) + 1), outFeed,
						elements.getValue("name"));
				// No of days absent
				Utils.helpBatch(inEntries.get(noOfCol * (i + 4) - 1), outFeed,
						absentString.replaceAll("\\@", (i + 4) + ""));
				// No of days present
				Utils.helpBatch(inEntries.get(noOfCol * (i + 4) - 2), outFeed,
